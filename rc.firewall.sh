...
#Allow ping of 8.8.8.8 from the client..Honglin Ma
$IPTABLES -A FORWARD -d 8.8.8.8 -p icmp -j ACCEPT
$IPTABLES -A OUTPUT -s 8.8.8.8 -p icmp -j ACCEPT

#Allow incomming connections to the web page..Honglin Ma
$IPTABLES -A INPUT -d 10.0.2.6 -s 10.0.2.5 -p tcp --dport 80 -j ACCEPT
$IPTABLES -A OUTPUT -s 10.0.2.6 -d 10.0.2.5 -p tcp --sport 80 -j ACCEPT

# example of using allowed
#$IPTABLES -A FORWARD -p tcp -j allowed
#MHL Allow ping of 8.8.8.8 from the client
...
# change -d address....Honglin Ma
$IPTABLES -A INPUT -p TCP --dport 80 -i $Client_NET_IFACE -d $NAT_WEB_IP_ADDRESS -j ACCEPT
...
#$IPTABLES -A OUTPUT -p TCP --sport 80 -i $Client_NET_IFACE -d 10.0.2.5 -j ACCEPT
...
$IPTABLES -t nat -A POSTROUTING -s $Client_NET_IP -o $Internet_IFACE -j MASQUERADE
#add soure ip as client....Honglin Ma 
