...
class Firewall(EventMixin):
    ...
    def _handle_PacketIn(self, event):
        ...
        s_mac = match.dl_src
        d_mac=packet.dst
        if (not self.match.getk(str(s_mac))) or (self.match.getk(str(s_mac)) and self.match[str(s_mac)] != str(s_ip) ):
            self.disbaled_MAC_pair.append(Eaddr(s_mac),EthAddr(d_mac))
